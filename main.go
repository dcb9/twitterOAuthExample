...
func New(config *Config) *http.ServeMux {
	...
	// mux.Handle("/twitter/callback111", twitter.CallbackHandler(oauth1Config, issueSession(), nil))
	// mux.Handle("/twitter/callback", twitter.CallbackHandler(oauth1Config, issueSession(), nil))

	mux.Handle("/twitter/login-url", generateTwitterLoginURLHandler(oauth1Config))
	mux.Handle("/twitter/user-info", getTwitterUserInfo(oauth1Config))
	return mux
}
